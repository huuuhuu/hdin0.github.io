...
function move( centerOfMass ) {
  console.log(centerOfMass); // returns 400,400,400
  calcChange();
let j = 3;
  for (let i = 0; i < count; i++ ) {
    console.log(centerOfMass); // returns NaN NaN NaN
    console.log(j);
...
function stayInGroup( boidIndex, cenOM ) {
  ...
  let r = Math.abs(b.distanceTo( cenOM ));
  // console.log(cenOM);

  if (r > flockRad) {
    let tocenOM = cenOM.sub(boidIndex);
    // console.log(1);
    let oldmag = boids.accel[boidIndex].length();
    tocenOM.normalize();
    tocenOM.multiplyScalar( oldmag );
